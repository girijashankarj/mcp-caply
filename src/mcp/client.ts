  ...
  private async request(method: string, params: Record<string, unknown> = {}): Promise<JsonRpcResponse> {
    ...
    try {
      ...
      if (error instanceof TypeError) {
        throw new Error('Browser could not reach this MCP endpoint. Check the URL, HTTPS, server CORS policy, and authentication configuration.');
      }
      throw error;
    } finally {
  ...
  async connect(): Promise<McpCatalog> {
    ...
    for (const version of PROTOCOL_VERSIONS) {
      ...
      try {
        ...
        if (!PROTOCOL_VERSIONS.includes(negotiated)) {
          throw new Error(`Server negotiated unsupported MCP protocol version ${negotiated}.`);
        }
